    ...
    <div className="blog-post">
      <div className="blog-post-splash">
        <div className="globe">
          ...
          {/* TODO: Either this needs to be an interactive globe that highlights the spot on the globe */}
          {/* TODO: The alternative would be to just save a country name, then have a SVG map where I can pass the country and its highlighted (if this exists) */}
        </div>
        <div>
          <h2>{title}</h2>
          <p>{description}</p>
      ...
      <div className="image-grid">
        {images.map((image) => {
          return (
            <img src={image.src} alt={`Day ${day} image`} className="blog-post-image" onClick={() => handleImageClick(image.id)} key={image.id} />
          )
        })}
      </div>
      ...
      <Modal show={showModal} onHide={handleClose} animation={false} className="portfolio-modal blog-post-modal">
        ...
        {/* TODO: Make a close button component, maybe store the svg as an svg as well */}
      </Modal>
    </div>
  )
}
